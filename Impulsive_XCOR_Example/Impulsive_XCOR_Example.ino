...
//            Serial.println(millis());
        ...
        Serial.println();
        normalize_vector(samples_mic0, BUFFER_SIZE);
        normalize_vector(samples_mic1, BUFFER_SIZE);
...
//        Serial.println(xcor_max);
        ...
        for (int i=0; i<BUFFER_SIZE; i++){
          Serial.print(*(samples_mic0+i));
          Serial.print(",");
          Serial.print(*(samples_mic1+i));
          Serial.println(";");
        }
        Serial.println("];");
...
{   
  samples_mic0 = (float*)malloc(4*BUFFER_SIZE);
  samples_mic1 = (float*)malloc(4*BUFFER_SIZE);
...
void samplingCallback() 
{
  ...
  *(samples_mic0+sample_index) = 0.1*((uint16_t)result.result_adc0 - pedestal); 
  *(samples_mic1+sample_index) = 0.1*((uint16_t)result.result_adc1 - pedestal);
...
void get_correlation(){
  arm_correlate_f32(samples_mic0, BUFFER_SIZE, samples_mic1, BUFFER_SIZE, xcor_sig);
